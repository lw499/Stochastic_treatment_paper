...
myfunc = function(m)
{
  ...
  afit = glm2(A ~ L1 + L2 + L1*L2 + W1 + W2 + I(abs(W2)), family = binomial(), data = dffull[dffull$lag_A==0,]) 
  ...
  cfit =  glm2(C~A+L1+I(sqrt(abs(W2*L2))) + I(abs(L2)/(1+exp(W2))) ,family=binomial,data=dffull)
  dffull$pred_obsc = predict(cfit, newdata = dffull, type="response")
  dffull$pred_obsc = ifelse(dffull$C==1, dffull$pred_obsc, 1-dffull$pred_obsc)
...
write.csv(test2,"ipw_haz_param.csv")
