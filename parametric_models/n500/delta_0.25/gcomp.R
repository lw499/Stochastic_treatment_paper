...
myfunc = function(m)
{
  ...
  #afit0 = glm2(A ~ L1 + L2 + L3, family = binomial(), data = dffull[dffull$t0==0,]) #This is from the data generation mechanism
  #afit = glm2(A ~ L1 + L2 + L3 + lag_A, family = binomial(), data = dffull) #This is from the data generation mechanism
  ...
  #dffull$pred_obs0 = predict(afit0, newdata = dffull, type="response")
  #dffull$pred_obs0 = ifelse(dffull$A==1, dffull$pred_obs0, 1-dffull$pred_obs0)
  #dffull$pred_obs = ifelse(dffull$t0==0, dffull$pred_obs0, dffull$pred_obs)
  #dffull$pred_obs0 = NULL; dffull$lag_A=NULL
  ...
  dffullwide = dcast(dffull, id ~ t0, value.var = c("L1","L2","L3","A","Cen","Y"))
  ...
  y4dat$A = y4dat$A_4; y4dat$L1 = y4dat$L1_4; y4dat$L2 = y4dat$L2_4; y4dat$L3 = y4dat$L3_4;
    ...
    y4dat$y4pred = predict(yfitog, newdata = y4dat, type="response")*(y4dat$L1_4*delta + 1-y4dat$L1_4) + predicttmp1*(1-delta)*(y4dat$L1_4); 
