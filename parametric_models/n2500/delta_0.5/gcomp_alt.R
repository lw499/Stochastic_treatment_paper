...
myfunc = function(m)
{
  ...
  afit = glm2(A ~ L1 + L2 + L3 + lag_A, family = binomial(), data = dffull) 
  ...
  cfit =  glm(Cen~L2+L3,family=binomial,data=dffull)
  ...
  dffullwide = dcast(dffull, id ~ t0, value.var = c("L1","L2","L3","A","Cen","Y","pred_obs","pred_obsc"))
  ...
  yfitog = glm2(Y~ (A+L1+L2+L3), family = binomial(), data = dffull[dffull$Y!=2,]) #This is from the data generation mechanism
  paramtmp = (yfitog)$coef
  
  ##################
  ######time 5######
  ##################
  #y4fit = glm2(Y_5 ~ A_4 + L1_4 + L2_4 + L3_4, family=binomial(), data = y4dat) ; 
  y4dat = tmpdata[tmpdata$Y_4<2 & tmpdata$Cen_3==0,]; #Cen_3 is really Cen_4
  y4dat$A = y4dat$A_4; y4dat$L1 = y4dat$L1_4; y4dat$L2 = y4dat$L2_4; y4dat$L3 = y4dat$L3_4;
  predicttmp1 = plogis(cbind(rep(1, nrow(y4dat)), rep(1, nrow(y4dat)), y4dat$L1_4, y4dat$L2_4, y4dat$L3_4)  %*% matrix(paramtmp, nrow=5))
  predicttmp0 = plogis(cbind(rep(1, nrow(y4dat)), rep(0, nrow(y4dat)), y4dat$L1_4, y4dat$L2_4, y4dat$L3_4)  %*% matrix(paramtmp, nrow=5))
  tmppart = (1-delta)
  tmppred = predicttmp1*(y4dat$pred_obs_4*delta + tmppart) + predicttmp0*(1-y4dat$pred_obs_4)*delta ##for L=1
  y4dat$y4pred = predict(yfitog, newdata = y4dat, type="response")*(1-y4dat$L1_4) + tmppred*y4dat$L1_4
  y4dat$y4pred = ifelse(y4dat$Y_4==0,0,y4dat$y4pred); 
  
  y4fit = glm2(y4pred ~ A_3*L1_3*L2_3*L3_3, family=binomial(), data = y4dat) ; #+ L1_3*L2_3 + L1_3*L3_3 + L2_3*L3_3 + A_3*L1_3 + A_3*L2_3 + A_3*L3_3
  y4dat = tmpdata[tmpdata$Y_3<2 & tmpdata$Cen_2==0,]; 
  tmp1= y4dat; tmp1$A_3=1; tmp0= y4dat; tmp0$A_3=0; 
  pred1 = predict(y4fit, newdata = tmp1, type="response"); pred0 = predict(y4fit, newdata = tmp0, type="response")
  tmppred = pred1*(y4dat$pred_obs_3*delta + tmppart) + pred0*(1-y4dat$pred_obs_3)*delta ##for L=1
  y4dat$y4pred = predict(y4fit, newdata = y4dat, type="response")*(1-y4dat$L1_3) + tmppred*y4dat$L1_3
  y4dat$y4pred = ifelse(y4dat$Y_3==0,0,y4dat$y4pred); 
  
  y4fit = glm2(y4pred ~ A_2*L1_2*L2_2*L3_2, family=binomial(), data = y4dat) ; # + L1_2*L2_2 + L1_2*L3_2 + L2_2*L3_2 + A_2*L1_2 + A_2*L2_2 + A_2*L3_2
  y4dat = tmpdata[tmpdata$Y_2<2 & tmpdata$Cen_1==0,]; 
  tmp1= y4dat; tmp1$A_2=1; tmp0= y4dat; tmp0$A_2=0 
  pred1 = predict(y4fit, newdata = tmp1, type="response"); pred0 = predict(y4fit, newdata = tmp0, type="response")
  tmppred = pred1*(y4dat$pred_obs_2*delta + tmppart) + pred0*(1-y4dat$pred_obs_2)*delta ##for L=1
  y4dat$y4pred = predict(y4fit, newdata = y4dat, type="response")*(1-y4dat$L1_2) + tmppred*y4dat$L1_2
  y4dat$y4pred = ifelse(y4dat$Y_2==0,0,y4dat$y4pred); 
  
  y4fit = glm2(y4pred ~ A_1*L1_1*L2_1*L3_1, family=binomial(), data = y4dat) ; # + L1_1*L2_1 + L1_1*L3_1 + L2_1*L3_1 + A_1*L1_1 + A_1*L2_1 + A_1*L3_1
  y4dat = tmpdata[tmpdata$Y_1<2 & tmpdata$Cen_0==0,]; 
  tmp1= y4dat; tmp1$A_1=1; tmp0= y4dat; tmp0$A_1=0; 
  pred1 = predict(y4fit, newdata = tmp1, type="response"); pred0 = predict(y4fit, newdata = tmp0, type="response")
  tmppred = pred1*(y4dat$pred_obs_1*delta + tmppart) + pred0*(1-y4dat$pred_obs_1)*delta ##for L=1
  y4dat$y4pred = predict(y4fit, newdata = y4dat, type="response")*(1-y4dat$L1_1) + tmppred*y4dat$L1_1
  y4dat$y4pred = ifelse(y4dat$Y_1==0,0,y4dat$y4pred); 
  
  y4fit = glm2(y4pred ~ A_0*L1_0*L2_0*L3_0, family=binomial(), data = y4dat) ; # + L1_0*L2_0 + L1_0*L3_0 + L2_0*L3_0 + A_0*L1_0 + A_0*L2_0 + A_0*L3_0
  y4dat = tmpdata
  tmp1= y4dat; tmp1$A_0=1; tmp0= y4dat; tmp0$A_0=0; 
  pred1 = predict(y4fit, newdata = tmp1, type="response"); pred0 = predict(y4fit, newdata = tmp0, type="response")
  tmppred = pred1*(y4dat$pred_obs_0*delta + tmppart) + pred0*(1-y4dat$pred_obs_0)*delta ##for L=1
  y4dat$y4pred = predict(y4fit, newdata = y4dat, type="response")*(1-y4dat$L1_0) + tmppred*y4dat$L1_0
...
write.csv(test2,"seq_reg_alt.csv")
