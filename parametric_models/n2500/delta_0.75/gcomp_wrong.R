...
myfunc = function(m)
{
  ...
  y4fit = glm2(y4pred ~ A_3 + L1_3 + L2_3 + L3_3, family=binomial(), data = y4dat)
  y4dat = tmpdata[tmpdata$Y_3<2 & tmpdata$Cen_2==0,]; 
    tmp0= y4dat; tmp0$A_3=0; tmp1= y4dat; tmp1$A_3=1; 
  ...
  y4fit = glm2(y4pred ~ A_2 + L1_2 + L2_2 + L3_2, family=binomial(), data = y4dat)
  y4dat = tmpdata[tmpdata$Y_2<2 & tmpdata$Cen_1==0,]; 
    tmp0= y4dat; tmp0$A_2=0; tmp1= y4dat; tmp1$A_2=1; 
    predicttmp1 = predict(y4fit, newdata = tmp1, type="response")
  ...
  y4fit = glm2(y4pred ~ A_1 + L1_1 + L2_1 + L3_1, family=binomial(), data = y4dat)
  y4dat = tmpdata[tmpdata$Y_1<2 & tmpdata$Cen_0==0,]; 
    tmp0= y4dat; tmp0$A_1=0; tmp1= y4dat; tmp1$A_1=1; 
    predicttmp1 = predict(y4fit, newdata = tmp1, type="response")
  ...
  y4fit = glm2(y4pred ~ A_0 + L1_0  + L2_0 + L3_0, family=binomial(), data = y4dat)
  y4dat = tmpdata
  tmp0= y4dat; tmp0$A_0=0; tmp1= y4dat; tmp1$A_0=1; 
    predicttmp1 = predict(y4fit, newdata = tmp1, type="response")
...
write.csv(test2,"seq_reg_wrong.csv")
